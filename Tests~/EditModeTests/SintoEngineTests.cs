...
[TestFixture]
public class SintoEngineTests
{
    [Test] public void Constructor_DoesNotThrow()
        => Assert.DoesNotThrow(() => { using var e = new SintoEngine(); });

    [Test] public void IsPaused_InitiallyFalse()
    {
        using var e = new SintoEngine();
    ...
    [Test] public void DefaultControlEvent_KindIsNone()
    {
        // default(ControlEvent) must have Kind=None (not NoteOn).
        // This is the first line of defense against phantom notes.
        var ev = default(ControlEvent);
        Assert.That(ev.Kind, Is.EqualTo(ControlEventKind.None));
    ...
    [Test] public void ProcessAudioCallback_NoneEvent_IsIgnoredSafely()
    {
        ...
        // The None event defense is: (1) default(ControlEvent).Kind == None proven above,
    ...
    [Test] public void SubBuffering_OffsetFrames_EventFiresAtCorrectPosition()
    {
        ...
        using var e = new SintoEngine(sampleRate: SR, channels: Channels, bufferSize: Frames);
