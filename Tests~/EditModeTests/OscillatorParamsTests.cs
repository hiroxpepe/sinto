...
[TestFixture]
public class OscillatorParamsTests
{
    [Test] public void OscillatorParams_IsValueType()
        => Assert.That(typeof(OscillatorParams).IsValueType, Is.True);

    [Test] public void Constructor_DefaultValues_AreValid()
    {
        var p = new OscillatorParams(WaveType.Sine);
        Assert.That(p.Wave,        Is.EqualTo(WaveType.Sine));
        Assert.That(p.Interp,      Is.EqualTo(InterpMode.Linear));
    ...
    {
        var p = new OscillatorParams(wave);
        Assert.That(p.Wave, Is.EqualTo(wave));
    }

    [TestCase(InterpMode.Linear)]
    [TestCase(InterpMode.NearestNeighbor)]
    public void Constructor_AllInterpModes_Stored(InterpMode interp)
    {
        var p = new OscillatorParams(WaveType.Sine, interp);
    ...
    {
        var p = new OscillatorParams(WaveType.Saw, detuneCents: input);
    ...
    {
        var p = new OscillatorParams(WaveType.Square, pulseWidth: input);
    ...
    {
        var p = new OscillatorParams(WaveType.Sine, level: input);
