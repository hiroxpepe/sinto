...
[TestFixture]
public class OscillatorStateTests
{
    const int SR = 44100;

    static OscillatorParams MakeParams(WaveType wave,
        InterpMode interp = InterpMode.Linear, float pw = 0.5f)
    ...
    [Test] public void NearestNeighbor_ProducesOutput()
    {
        ...
        var osc = new OscillatorState();
        osc.SetFrequency(440f, SR);
        var p = MakeParams(WaveType.Sine, InterpMode.NearestNeighbor);
        float s = osc.Tick(p);
