...
public struct SmoothedParameter {
    ...
    private long  _sampleIndex; // for DenormalGuard parity — SmoothedParameter is a 1-pole IIR
    ...
    // Implementation must apply DenormalGuard or Sleep optimization:
    //   if (MathF.Abs(_target - _current) < 1e-5f) { _current = _target; return _current; }
    //   else { _current += (_target - _current) * _coeff + DenormalGuard.Protect(0f, _sampleIndex++); }
